...
- **AllShipsAllSlots** - A lightweight (10KB) mod that adds both Forward and AFT weapon slots to every ship in Sunless Sea. Features configurable settings and compatibility with vanilla and modded ships across all game versions (Steam, Epic, GOG). Serves as a more efficient replacement for the original "Aft Slots for ALL" mod. Feature-complete. [**Nexusmods**](https://www.nexusmods.com/sunlesssea/mods/55)
- **EFSETSG** - Similar to AllShipsAllSlots, this is a BepInEx port of Mr4Iter's Engines Fuel Spend Equals Speed Gain. Replaces the original edited-DLL release with a Harmony patch for legality, cross-store compatibility, and coexistence with other mods. Feature-complete. [**GitHub**](https://github.com/MagicJinn/SSea-EFSETSG-BepInEx)
...
Chronos Backups is a multi-loader, multi-version Minecraft backup utility that keeps your world safe without filling your disk. Instead of backing up every chunk, it intelligently prunes backups to include only the most important parts of your world, dramatically reducing backup size while preserving the areas that actually matter. A shared core with loader-specific shells lets it run on Fabric, NeoForge, Forge, Bukkit, Spigot, Paper, Purpur and Folia from 1.7.x through 26.x, making it the broadest version range of any Minecraft backup mod (or possibly any mod). The mod is completely serverside, so vanilla clients can still play while the mod is installed.
...
**Technologies: Minecraft Modding, Fabric, NeoForge, Forge, Bukkit/Paper, Native Rust FFI, Google Drive Sync, Docker Integration Testing, GitHub Actions, CI/CD**<br>
